...
defineEmits(["open", "select", "toggle-mute", "toggle-pin"]);
</script>
...
<template>
  ...
  >
    ...
    <div class="qq-thread-actions">
      <button class="qq-thread-action-btn" type="button" :title="row.isMuted ? '取消免打扰' : '消息免打扰'" @click.stop="$emit('toggle-mute', row)">
      ...
      <button class="qq-thread-action-btn" type="button" :title="row.pinnedAt ? '取消置顶' : '置顶会话'" @click.stop="$emit('toggle-pin', row)">
        <svg viewBox="0 0 24 24" aria-hidden="true"><path d="m14 4 6 6-2 2-2-2-3 3v6l-2 1v-7L8 10l2-2 2 2 3-3-2-2 1-1Z"/></svg>
      </button>
      <button v-if="desktop" class="qq-thread-action-btn" type="button" title="打开独立窗口" @click.stop="$emit('open', row.id)">
